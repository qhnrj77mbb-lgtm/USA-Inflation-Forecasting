...
server <- function(input, output, session) {
  ...
  resultados <- reactive({
    ...
    if (input$tipo_modelo == "arima" || is.null(input$exogenas) || length(input$exogenas) == 0) {
      ...
      # Proyectar cada variable exógena con su propio ARIMA
      ...
      # Construir la matriz de regresores futuros (h filas × k columnas)
    ...
    # ── Reconstrucción del CPI y cálculo YoY ─────────────────────────────────
